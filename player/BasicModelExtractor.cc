...
BasicTeamModel* BasicModelExtractor::extractModel(TrajectoryData &trajectories, BasicStateProcessor &state_processor) {
    ...
    // Once the trajectories have been processed, extract the model
    for (int agentID = 0; agentID < numTakers; agentID++) {
        ...
        auto agent = make_unique<BasicAgentModel>(agentID, teamIDs, agentStateFile, agentArgFile, teamStateFiles,
...
BasicTeamModel* BasicModelExtractor::extractModel(TrajectoryData &trajectories) {
...
//   //   allActionArguments.push_back(make_unique<ArgumentT2H4>(offset++, 1, 0, numTakers, numKeepers));
