...
taker_load_dir="good_weights"    # sub-directory of weight_dir where weights are stored
taker_learn=1                    # should learning be turned on for takers?
taker_policy="learned"       # policy followed by takers {learned, learned!, hand, hold, random} (BASELINE!!!! (unless sarsa))
taker_mode="sarsa"         # the real taker mode, the previous one only sets the baseline (see main.cc & the Analisys class)
                                 # {handcrafted, sarsa, extracted, default, orla}
...
start_udp=0                      # should I wait for a "start" udp message before (re)starting an episode?
episode_timeout=0                # should I end the episode if the takers do not pass the ball in a long time?

ka_width=40                      # Y-axis size of playing region
...
vision_opts="$s::visible_angle=720 $s::stamina_max=9999999"

#  vision_opts="$s::visible_angle=360"

fi

server_opts="$s::port=$port $s::coach_port=$coach_port $s::olcoach_port=$olcoach_port $s::half_time=-1 $s::forbid_kick_off_offside=0 $s::use_offside=0 $s::stamina_inc_max=1000000 $s::synch_mode=$synch_mode $s::synch_offset=80 $keepaway_opts $log_opts $vision_opts $s::coach=$use_trainer $s::start_udp=$start_udp $s::episode_timeout=$episode_timeout"

keeper_opts="-t keepers -e $keeper_learn -q $keeper_policy -z $taker_mode"
taker_opts="-t takers -e $taker_learn -q $taker_policy -z $taker_mode"
...
do
  ...
  tcmd_line="./$client $client_opts $taker_opts $tlog_opts $tdraw_opts $tweight_opts -g $i"
